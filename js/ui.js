...
function showAdditionalStats(fleet) {
	...
	for (var i=0; i<ships.length; i++) {
		...
		if (fleet == 12 || fleet == 13){
			td.append('<span>Shell Power: '+Math.floor(ships[i].shellPower(undefined, -1, true))+'</span><br>');
		}else{
			...
			if (ships[i].canASW()) {
				td.append('<span>ASW Power: '+Math.floor(ships[i].ASWPower())+(ships[i].canOASW()? ' (OASW)':'')+'</span><br>');
			}
		}
	}
	table.append(tr);
		...
		for (var i=0; i<ships.length; i++) {
			...
			if (ships[i].NBtypes().length) {
				...
				for (let NBtype of ships[i].NBtypes()) {
					...
					if (ships[i].type == 'DD' && ships[i].LVL >= 80 && typedata.improve) {  // for DD CI
						let chanceMod = typedata.chanceMod;
						chance /= chanceMod;
						let improveChance = typedata.improveChance;
						name = NBATTACKDATA[typedata.improve].name;
						td.append('<div style="margin-left:16px">'+name+': '+Math.floor(100*chance*improveChance*chanceleft)+'%</div>');
						name = typedata.name;
						td.append('<div style="margin-left:16px">'+name+': '+Math.floor(100*chance*(1-improveChance)*chanceleft)+'%</div>');
					}else {
						let chanceMod = typedata.chanceMod;
...
function genFleetHTML(rootid,fleetnum,fleetname,tabcolor,isCombined,isSupport,addEnemyCombined) {
	...
	if (fleetnum == 1 || fleetnum == 11){
		...
		for (var j=0; j<6; j++) {
			...
			text.appendChild(document.createTextNode('Bonus A: '));
			td.appendChild(text);
			var input = document.createElement('input');
			input.setAttribute('type','number'); input.setAttribute('id',tid+'bonusA'+j);
		...
		for (var j=0; j<6; j++) {
			...
			var text = document.createElement('div'); text.setAttribute('title',desc);
			text.setAttribute('style','float:left');
			text.appendChild(document.createTextNode('Bonus B: '));
			td.appendChild(text);
			var input = document.createElement('input');
		...
		for (var j=0; j<6; j++) {
			...
			var text = document.createElement('div'); text.setAttribute('title',desc);
			text.setAttribute('style','float:left');
			text.appendChild(document.createTextNode('Bonus C: '));
			td.appendChild(text);
			var input = document.createElement('input');
			input.setAttribute('type','number'); input.setAttribute('id',tid+'bonusC'+j);
			input.setAttribute('min','0'); input.setAttribute('max','99'); input.setAttribute('step','.1'); input.setAttribute('title',desc);
			input.setAttribute('onchange','updateFleetCode('+fleetnum+')');
			input.setAttribute('style','width:65px');
			td.appendChild(input);
		...
		for (var j=0; j<6; j++) {
			...
			var text = document.createElement('div'); text.setAttribute('title',desc);
			text.setAttribute('style','float:left');
			text.appendChild(document.createTextNode('Debuff: '));
			td.appendChild(text);
			var input = document.createElement('input');
			input.setAttribute('type','number'); input.setAttribute('id',tid+'debuff'+j);
			input.setAttribute('min','0'); input.setAttribute('max','99'); input.setAttribute('step','.1'); input.setAttribute('title',desc);
			input.setAttribute('onchange','updateFleetCode('+fleetnum+')');
			input.setAttribute('style','width:65px');
			td.appendChild(input);
		...
		for (var j=0; j<6; j++) {
			...
			var text = document.createElement('div'); text.setAttribute('title',desc);
			text.setAttribute('style','float:left');
			text.appendChild(document.createTextNode('Bonus: '));
			td.appendChild(text);
			var input = document.createElement('input');
			input.setAttribute('type','number'); input.setAttribute('id',tid+'bonusA'+j);
			input.setAttribute('min','0'); input.setAttribute('max','99'); input.setAttribute('step','.1'); input.setAttribute('title',desc);
...
function genOptions(fleetnum) {
	...
	div.append('<span class="option2"> <img src="assets/stats/fuel.png" /> <input type="number" id="fuelloss'+fleetnum+'" min="0" max="100" value="0" style="width:50px" onchange="updateOptionsCookies('+fleetnum+');raiseFleetChange()"/>%</span>');
	div.append('<span class="option2"> <img src="assets/stats/ammo.png" /> <input type="number" id="ammoloss'+fleetnum+'" min="0" max="100" value="0" style="width:50px" onchange="updateOptionsCookies('+fleetnum+');raiseFleetChange()"/>%</span>');
	...
	div.append('<span class="option2"><label>Battle Stage: </label></span>');
	...
	div.append('<span class="option2"><input type="number" id="engagemod'+fleetnum+'" min="0.6" max="1.2" value="1" style="width:50px" onchange="updateOptionsCookies('+fleetnum+');raiseFleetChange()"/></span>');
	...
	div.append('<span class="option2"><label>Ship historical bonus </label></span>');
...
function extractOptions(num) {
	...
	if (ADDEDLBAS) {
		...
		options.lbraid = $('#lbraid'+num).prop('checked');
	}
...
function loadOptions(num,options) {
	...
	if (options.lbraid) $('#lbraid'+num).prop('checked',options.lbraid);
	...
	if (equipid) {  //add new equip
		...
		if (fleet == 'LB'){
			...
			if (equipid == 311) {
				...
				$('#T'+fleet+'prof'+slot+equipslot).val(2);
				changedProficiency('#T'+fleet+'prof'+slot+equipslot);
			}
		}
...
function loadIntoSim(fleet,side,isescort) {
	...
	for (var i=0; i<6; i++) {
		...
		if (mid) {
			...
			if (document.getElementById('T'+fleet+'bonusA'+i)) {
				s.bonusA = parseFloat(document.getElementById('T'+fleet+'bonusA'+i).value) || 0;
			}
...
var ADDEDLBRAID = false;
function clickedAddLBRaid(update) {
	if (ADDEDLBRAID) return;
	if (!document.getElementById('T99')) genFleetHTML('fleetspace2Raid', 99, 'Land Base Raid', '#ffaaaa');
	else $('#T99').css('display','block');
	ADDEDLBRAID = true;
	if (update) updateFleetCode('99');
	$('#btnDelLBRaid').css('display','');
	$('#btnAddLBRaid').css('display','none');
	raiseFleetChange();
}

function clickedDelLBRaid() {
	if (!ADDEDLBRAID) return;
	$('#T99').css('display','none');
	saveFleet('99','');
	ADDEDLBRAID = false;
	$('#btnDelLBRaid').css('display','none');
	$('#btnAddLBRaid').css('display','');
	raiseFleetChange();
}
...
function extractForSim() {
	...
	if (ADDEDLBAS) {
		...
		if (ADDEDLBRAID) {
			d = loadIntoSim(99,1);
			if (!d[0].length) FLEETLBRAID = null;
			else {
				FLEETLBRAID = new Fleet(1);
				FLEETLBRAID.loadShips(d[0]);
				FLEETLBRAID.formation = ALLFORMATIONS[d[1]];
			}
		}else {
			FLEETLBRAID = null;
		}
...
function loadLocalStorage() {
	...
	if (localStorage.simulator_fleet99) clickedAddLBRaid();
